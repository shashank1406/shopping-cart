...
const createOrder = async function (req, res) {
    try {
        ...
        if (!isValid(items)) {
            return res.status(400).send({ status: true, message: 'items is required in the request body' })
        }
        if (!isValid(totalPrice)) {
            return res.status(400).send({ status: true, message: 'totalPrice is required in the request body' })
        }
        if (!isValid(totalItems)) {
            return res.status(400).send({ status: true, message: 'totalItems is required in the request body' })
        }
        if (!isValid(totalQuantity)) {
            return res.status(400).send({ status: true, message: 'totalQuantity required in the request body' })
        }
